...
delay=$(kubectl -n$NS get pod probe -o=jsonpath="{$.spec.containers[?(@.name=='probe')].livenessProbe.initialDelaySeconds}" 2>/dev/null)
...
if [[ $delay != "$DELAY" ]]; then
  error=true
  echo "the 'initial liveness probe delay' is wrong"
#echo found: $port
fi
